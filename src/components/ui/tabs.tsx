...
export function Tabs({ children, defaultValue, value, onValueChange, className }: { children: React.ReactNode; defaultValue?: string; value?: string; onValueChange?: (value: string) => void; className?: string }) { const s = useStyles(); return <div className={[s.root, className].filter(Boolean).join(" ")}><div data-tabs-value={value ?? defaultValue} onClick={(e) => { const target = (e.target as HTMLElement).closest("[data-value]"); if (target) onValueChange?.(target.getAttribute("data-value") || ""); }}>{children}</div></div>; }
